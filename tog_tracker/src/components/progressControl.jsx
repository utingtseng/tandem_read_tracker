...
const ProgressControl = () => {
    ...
    return (
        <>
        {books? bookOrder.map((order) => {
            const book = books[order];
            return (<BookSection order={order} book={book} setBooks={setBooks}/>)
        }): null}
        </>
    )
}
...
const BookSection = ({book, order, setBooks}) => {
    ...
    <div className="book-card">
        ...
        {book.comments && book.comments.length >= 1 && <p><strong>Comments:</strong><BookComments comments={book.comments}/></p>}
        <br></br>
        {order == "curr" ? 
        <>
        <Row>
            ...
            <Col><Button onClick={handleAddNewComment}>+ Comments</Button></Col>
        </Row>
        <Button onClick={handleMarkComplete}>Mark as completed!</Button></> : null}
        </div>
    </div>
    )
}
